...
- One repo-local deploy command per delivery target, such as `deploy-preview`, `deploy-beta`, or `deploy-production`, so workflow YAML stays thin. Do not overload one generic `deploy`
...
- Use the smallest set of secrets and permissions each deploy target requires.
- Deliver beta or preview builds continuously. Add stricter promotion gates only where the product or platform requires them. No manual checklist-driven deploy flow unless the platform requires it.
