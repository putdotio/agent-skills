...
- Use full-SHA-pinned `pnpm/action-setup`, then pinned `actions/setup-node` with `cache: pnpm`, then `pnpm install --frozen-lockfile` for TypeScript repos that do not use Vite+ (`vp`)
