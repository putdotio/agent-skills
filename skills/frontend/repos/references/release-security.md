...
- Prefer repo settings that make the intended refs real: restrict `main` to the put.io team, protect release tags, require reviewers on release environments, and consider action SHA pinning on release workflows

## Repo Settings Model

- The default put.io posture is trusted-team direct pushes to `main`, strict release/deploy boundaries
- Public repos: protect `main` so only the put.io team can push, while force-pushes and branch deletes stay blocked. Do not require PRs on `main` unless the repo owner wants that workflow
...
- Release tags such as `v*` should be created by trusted release automation or release admins only. If semantic-release or a similar workflow creates tags, allow that workflow identity and block arbitrary manual tag creation or mutation
...
- If using rulesets instead, avoid a broad bypass that also permits force-push or delete. Prefer a no-bypass baseline rule for deletion/force-push protection plus a narrow update rule that only the put.io team can bypass
...
- release tag protection or rulesets for `v*`
