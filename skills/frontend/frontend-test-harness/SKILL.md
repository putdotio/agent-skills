...
7. Design auth/session setup to be fully autonomous: one bounded 1Password materialization step, then typed `putio` profile, device approval, authorization, seeding, and reset commands for the testing account.
8. Do not rely on repeated human 1Password approvals, browser `put.io/link` completion, the human's browser session, or personal put.io accounts for routine harness work.
...
11. Before implementation, check the design covers the harness layers from `harness-pattern.md`: adapter, CLI/API, auth/session, flow driver, assertions, proof artifacts, repo integration, observability, isolation, and boundaries.
